---
name: faigate
description: fusionAIze Gate routing stats and control. Use when the user asks about API costs, model routing, token usage, cache hit rates, provider health, or wants to test how a prompt would be routed. Commands — /faigate stats, /faigate route, /faigate health, /faigate daily.
metadata: {"openclaw":{"requires":{"bins":["curl"]},"emoji":"🚪","homepage":"https://github.com/fusionAIze/faigate"}}
---

# fusionAIze Gate Skill

fusionAIze Gate is a local routing proxy that sits between OpenClaw and your model providers (chat and image-capable backends).

OpenClaw note: when OpenClaw talks to fusionAIze Gate, the model ids in the OpenClaw config must match the provider ids returned by `GET /v1/models` from fusionAIze Gate. Those ids are local fusionAIze Gate ids such as `auto`, `deepseek-chat`, `local-worker`, or `image-provider`, not raw upstream model names.
...
### /faigate image <prompt>
Send one real image-generation request through fusionAIze Gate.
...
fusionAIze Gate uses 6 routing stages for chat requests (evaluated in order, first decisive match wins):
...
- Prefer one OpenClaw provider named `faigate` that points to `http://127.0.0.1:8090/v1`.
- Use `faigate/auto` as the default `model.primary` unless you want to pin one explicit fusionAIze Gate provider.
- Use `faigate/auto` for `imageModel.primary` when fusionAIze Gate should pick among image-capable providers. Pin `faigate/<provider-id>` only when the image backend must be fixed.
- If many-agent traffic should be distinguishable, keep `x-openclaw-source` short and stable.
- If you want fusionAIze Gate to classify OpenClaw traffic automatically, enable `client_profiles.presets: ["openclaw"]` in `config.yaml`.

## Prompt Caching

DeepSeek and Gemini automatically cache repeated prefixes server-side. fusionAIze Gate tracks cache hit/miss tokens in metrics. To maximize cache hits:
